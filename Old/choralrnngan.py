...
from RNNAttention import get_distinct, create_lookups, prepare_sequences, \
...
def trainmodel(run_id, music_name, seq_len=32, mode='build'):
    # run params
    section = 'compose'
    # run_id = '0006'
    # music_name = 'paganini'

    run_folder = 'Data/Generated/ChoralRNNGAN/{}/'.format(section)
    ...
    if mode == 'build':
        print("Iterating through pieces...")
        ...
        for i, file in enumerate(music_list):
            ...
            try:
                ...
                print("Error occurred while parsing file: %s" % file)
    ...
    if not os.path.isfile('Images/Choral_rnn_gan.png'):  # keras plot_model library
        plot_model(model, to_file='Images/Choral_rnn_gan.png', show_shapes=True, show_layer_names=True)
...
def predictmodel(run_id, music_name, max_seq_len=32, max_extra_notes=512):
    ...
    fig.savefig('Images/ChoralRNN_GAN_sequence_plot.png')
...
def svmdiscriminator(generated_midi_path, csvname, numdims, verbose=False, nondefpath=False, pathtocheck=""):
    ...
    if os.path.isfile(csvpath):
        ...
        with open("Data/Tabular/{}.csv".format(csvname), "w", newline='') as f:
            writer = csv.writer(f)
...
if __name__ == '__main__':
    # viewscore('24')
    # trainmodel('0001', 'Data/MIDI/VoiceParts/Combined', mode='load')  # , mode='load')
    for i in range(0, 5):
        predictmodel('0001', 'Data/MIDI/VoiceParts/Combined', max_seq_len=int(random.randint(32, 128) / 32) * 32)
    # for i in range(0, 30):
    #     predictmodel('0001', 'liedersample')
    # for i in range(0, 30):
